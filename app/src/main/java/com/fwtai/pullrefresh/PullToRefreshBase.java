...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
			...
			case MotionEvent.ACTION_UP: {
				if (isBeingDragged) {
					...
					if (state == RELEASE_TO_REFRESH && null != onRefreshListener) {
						scrollTo(0, currentMode == MODE_PULL_DOWN_TO_REFRESH ? -headerHeight : headerHeight);
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (a.hasValue(R.styleable.PullToRefresh_mode_common)) {
			mode = a.getInteger(R.styleable.PullToRefresh_mode_common, MODE_PULL_DOWN_TO_REFRESH);
		}

		// Loading View Strings
		String pullLabel = context.getString(R.string.pull_down_to_refresh_pull_label);
		...
		if (mode == MODE_PULL_DOWN_TO_REFRESH || mode == MODE_BOTH) {
			...
			addView(headerLayout, new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT));
		...
		pullLabel = context.getString(R.string.pull_up_to_refresh_pull_label);
...
//		    this.setBackgroundResource(Color.WHITE);
		}
		if (a.hasValue(R.styleable.PullToRefresh_adapterViewBackground)) {
			refreshableView.setBackgroundResource(a.getResourceId(R.styleable.PullToRefresh_adapterViewBackground,
					Color.WHITE));
//		    refreshableView.setBackgroundResource(Color.WHITE);
		}
		a.recycle();
		// Hide Loading Views
		switch (mode) {
